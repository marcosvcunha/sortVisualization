...
class Button():
    def __init__(self, pygame, win, onPressed=None, x=0, y=0, height=0, width=0, text = '', color=(255,255,255)):
    ...
    def setText(self, text):
        self.text = self.myFont.render(text, False, (0,0,0), self.color)
        if(self.width < self.text.get_width()):
            self.width = self.text.get_width()

    def draw(self):
        self.pygame.draw.rect(self.win, self.color, (self.x - self.width/2, self.y - self.height/2, self.width, self.height))
